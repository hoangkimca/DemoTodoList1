...
export const changeTodosAsync = createAsyncThunk(
    'todos/changeTodosAsync',
    async (payload) => {
        const response = await fetch(`http://localhost:7000/todos/change/${payload.id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
...
const todoSlice = createSlice({
    ...
    extraReducers: {
        ...
        [changeTodosAsync.fulfilled]: (state, action) => {
            const index = state.findIndex((todo) => todo.id === action.payload.id);
            console.log(index);
